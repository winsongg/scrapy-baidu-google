...
class amazon(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        sql = "select bigname,keyword from tb_dekeywords where isvalid=1"
        key = "search_dekeypool"
        try:
            cursor.execute(sql)
        ...
        while rm.getvalue(key) != None and len(rm.getvalue(key))!=0:
            dict = eval(rm.popredis(key,"search_dedealpool"))
            keyword = dict[1].replace("&","%26")
            start_url = "https://www.amazon.de/s/ref=sr_nr_p_n_availability_1?rh=n:"+params[dict[0]][0]+",k:"+keyword+",p_72:"+params[dict[0]][2]+",p_n_availability:"+params[dict[0]][1]+"&keywords="+keyword+"&ie=UTF8"
