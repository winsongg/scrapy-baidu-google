# -*- coding: utf-8 -*-
import scrapy
from googleprj.items import DmozItem,deurl
...
class google(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        #填充google要爬的关键词
        if rm.getvalue("googledekeyword") == None or len(rm.getvalue("googledekeyword"))==0:
            sql = "select keyword from tb_dekeywords where isvalid=1"
            cursor.execute(sql)
            ...
            for row in keywords:
                kk.append(row[0])
            print kk
            rm.pushredis("googledekeyword",kk)
        
        key = rm.popredis("googledekeyword","googledekeyword_dealpool")
        ...
        if last == None:
                ...
                url = 'https://www.google.com/search?q=site%3Aamazon.de+'+key.replace(" ", "+").replace("&","%26")+'+currently+unavailable&oq=site%3Aamazon.co.de+'+key.replace(" ", "+").replace("&","%26")+'+currently+unavailable&aqs=chrome..69i57j69i58j69i60.3924j0j8&sourceid=chrome&ie=UTF-8'
                print 'kkkkk'+url
    ...
    def parse(self, response):
        ...
        #try:
            ...
            for one in list:
                ...
                if "https://www.amazon.de" in real_url or "http://amazon.de" in real_url:
                    keytemp = response.request.url
                    keytemp = keytemp[keytemp.find('amazon.de')+10:keytemp.find('currently')].replace("+"," ").replace("%27","").replace("%26","&").replace("'","")
